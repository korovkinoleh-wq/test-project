...
If you take a task, you must return without waiting for a user ping with one of three things:
1. the finished result
2. a concrete blocker
3. a brief status update by the agreed time if the task is long

Forbidden:
- saying you are working when there is no real execution
...
Missing the self-imposed return time counts as execution failure, not normal behavior.
